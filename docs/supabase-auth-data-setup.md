...
- `ADMIN_NOTIFICATION_WECOM_WEBHOOK` 或 `WECOM_BOT_WEBHOOK_URL`
- `ADMIN_NOTIFICATION_FEISHU_WEBHOOK` 或 `FEISHU_BOT_WEBHOOK_URL`
- `ADMIN_NOTIFICATION_FEISHU_SECRET` 或 `FEISHU_BOT_WEBHOOK_SECRET`
- `NEXT_PUBLIC_SITE_URL`

现在管理员通知不只覆盖“完成注册资料”，还会覆盖：

- 新加入申请提交
- 新合作线索提交

建议至少配置一条邮件通知，团队协作时再额外接企业微信或飞书机器人。
