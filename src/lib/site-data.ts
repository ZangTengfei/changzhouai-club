export const navItems = [
  { href: "/", label: "首页" },
  { href: "/events", label: "活动" },
  { href: "/news", label: "AI 资讯" },
  { href: "/members", label: "成员地图" },
  { href: "/works", label: "案例库" },
  { href: "/projects", label: "项目协作" },
  { href: "/cooperate", label: "合作联系" },
  { href: "/docs", label: "社区文档" },
];

export const siteRepositoryUrl = "https://github.com/ZangTengfei/changzhouai-club";

export const communitySocialLinks = [
  {
    platform: "小红书",
    label: "小红书",
    description: "关注社区图文动态",
    href: "https://www.xiaohongshu.com/user/profile/6982189c0000000021010bff",
    tone: "rednote",
    iconSrc: "/xiaohongshu.png",
  },
  {
    platform: "抖音",
    label: "抖音",
    description: "关注社区短视频",
    href: "https://v.douyin.com/_XyXh0JRN6U/",
    tone: "douyin",
    iconSrc: "/douyin.png",
  },
  {
    platform: "B站",
    label: "B 站",
    description: "关注社区视频空间",
    href: "https://space.bilibili.com/3706970819267057",
    tone: "bilibili",
    iconSrc: "/bilibili.png",
  },
] as const;

export const homeHighlights = [
  {
    title: "线下交流",
    description:
      "围绕常州本地的 AI 讨论、实操分享和自由交流，持续建立真实连接。",
  },
  {
    title: "OPC 与独立实践",
    description:
      "关注 AI 时代的一人产品、独立开发、自动化服务与高杠杆个体实践，帮助更多想法走向真实验证。",
  },
  {
    title: "企业合作",
    description:
      "承接主题分享、培训、PoC 验证和项目协作，把社区能力对接到真实场景。",
  },
];

export const activityMoments = [
  {
    title: "持续线下碰面",
    description:
      "社区保持稳定的线下活动节奏，让成员之间持续建立真实连接。",
  },
  {
    title: "成员开始主动分享",
    description:
      "活动中不断出现来自成员的一线实践分享，形成更有温度的交流氛围。",
  },
  {
    title: "官网承接内容沉淀",
    description:
      "官网持续沉淀活动回顾、成员信息与合作入口，帮助更多人快速了解社区。",
  },
];

export const projectStatus = [
  {
    title: "项目协作处于起步阶段",
    description:
      "社区当前以活动交流、需求对接和成员连接为主，围绕真实政企与企业需求逐步沉淀协作基础。",
  },
  {
    title: "真实需求正在进入社区",
    description:
      "活动和合作入口里已经陆续出现政企项目、企业场景和成员资源线索，帮助社区识别值得推进的方向。",
  },
  {
    title: "合作将围绕明确角色展开",
    description:
      "当项目目标、需求边界和参与角色清晰后，社区会开放对应的招募入口，方便成员申请参与。",
  },
];

export const opcTracks = [
  {
    title: "个人产品与微型 SaaS",
    description:
      "围绕独立产品、垂直工具和微型 SaaS，讨论从想法验证到上线迭代的真实路径。",
  },
  {
    title: "AI 自动化服务",
    description:
      "聚焦基于 Agent、工作流和行业知识的自动化服务，探索高杠杆交付与持续复用。",
  },
  {
    title: "内容、咨询与轻交付",
    description:
      "关注 AI 时代个体如何把经验、内容和咨询能力产品化，形成更稳定的业务结构。",
  },
];

export const memberTags = [
  "LLM 应用",
  "Agent",
  "RAG / 知识库",
  "OPC",
  "独立开发",
  "个人产品",
  "微型 SaaS",
  "前端工程",
  "后端服务",
  "自动化工作流",
  "产品设计",
  "AI 自动化服务",
  "咨询交付",
  "运营增长",
  "企业信息化",
  "高校交流",
  "本地创业",
  "项目交付",
];

export const joinSteps = [
  "使用邮箱或 Google 登录，创建你的社区账号",
  "先完成显示名和微信号这两个必填项",
  "在个人页持续补充资料、报名活动和参与协作",
];

export const cooperationAreas = [
  "AI 主题分享",
  "企业内训",
  "需求讨论",
  "PoC 验证",
  "项目协作",
  "人才连接",
];

export const archiveItems = [
  {
    title: "活动照片与回顾",
    description:
      "沉淀每场活动的时间、照片与现场氛围，帮助更多人理解社区的交流节奏。",
  },
  {
    title: "成员分享线索",
    description:
      "整理活动中出现过的分享主题、自研项目与讨论方向，呈现社区关注的话题。",
  },
  {
    title: "资料与记录",
    description:
      "汇集资料链接、嘉宾介绍与活动记录，形成社区可持续查阅的内容档案。",
  },
];

export const faqItems = [
  {
    question: "这个社区只面向开发者吗？",
    answer:
      "不是。开发者会是重要群体，但产品、设计、运营、高校同学、创业者和企业数字化从业者都适合加入。",
  },
  {
    question: "活动一定是技术分享吗？",
    answer:
      "不一定。活动可以是主题分享、圆桌讨论、自由交流、需求沟通会，也可以是项目想法交流。",
  },
  {
    question: "社区已经有正式项目了吗？",
    answer:
      "项目协作页会逐步展示经过社区确认、适合公开招募或申请对接的项目机会。未公开的合作线索仍会先由社区后台跟进。",
  },
  {
    question: "网站支持在线提交申请和合作需求吗？",
    answer:
      "支持。加入社区现在统一通过登录后完善个人资料完成；合作需求仍然可以直接在线提交。",
  },
];
