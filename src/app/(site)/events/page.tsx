...
export default async function EventsPage({
  ...
  return (
    <div className="grid gap-7 max-sm:gap-[22px]" data-events-page>
      ...
      <section className="grid grid-cols-[minmax(0,0.86fr)_minmax(440px,1.14fr)] items-start gap-8 pt-6 pb-2 max-lg:grid-cols-1 max-sm:pt-0" aria-labelledby="events-hero-title">
        ...
        <div className="relative min-h-[548px] pt-6 pb-20 pl-7 max-lg:min-h-[500px] max-lg:pt-7 max-lg:pb-[82px] max-lg:pl-5 max-[820px]:min-h-0 max-[820px]:pt-8 max-[820px]:pb-0 max-[820px]:pl-0">
          ...
          <article className="absolute right-[clamp(-22px,-2vw,-8px)] bottom-[-6px] z-[2] grid w-[min(390px,82%)] gap-3.5 rounded-[var(--radius-lg)] bg-[#e9f9f0] px-6 py-[22px] shadow-[0_18px_36px_rgba(var(--ink-rgb),0.1)] max-lg:right-0 max-lg:bottom-2 max-[820px]:relative max-[820px]:right-auto max-[820px]:bottom-auto max-[820px]:mt-[-42px] max-[820px]:w-full max-sm:p-5">
            ...
            <Link className="inline-flex w-fit items-center gap-2 font-black text-primary [&_svg]:size-4" href={nextEvent ? `/events/${nextEvent.slug}` : "#upcoming"} prefetch={false}>
      ...
      <section className="grid grid-cols-[minmax(260px,0.88fr)_minmax(300px,1fr)_auto] items-center gap-[18px] rounded-[var(--radius-lg)] bg-white p-[22px] shadow-[0_14px_34px_rgba(var(--ink-rgb),0.07)] max-lg:grid-cols-1 max-sm:p-5" aria-labelledby="event-proposal-title">
        <div className="grid min-w-0 gap-2 [&_.home-kicker]:bg-[#fff2e5]">
          ...
          <p className="m-0 text-[0.94rem] leading-[1.68] font-[650] text-[rgba(var(--ink-rgb),0.66)]">
          ...
          {eventProposalNotes.map((item, index) => {
            ...
            return (
              <article className={cn("grid min-w-0 grid-cols-[34px_minmax(0,1fr)] gap-x-2.5 gap-y-1 rounded-[var(--radius-md)] p-3.5", index === 0 ? "bg-[#fff2e5]" : "bg-[#f3efff]")} key={item.title}>
                ...
                <span className="text-[0.84rem] leading-[1.55] font-[650] text-[rgba(var(--ink-rgb),0.62)]">{item.summary}</span>
              </article>
            );
          })}
        </div>
        ...
        {completedEvents.length > 0 ? (
          <div className="grid gap-[18px]">
            {completedEvents.map((item, index) => (
              <article className="grid min-h-[278px] grid-cols-[minmax(130px,0.28fr)_minmax(260px,0.72fr)_minmax(0,1fr)] overflow-hidden rounded-[var(--radius-lg)] bg-white shadow-[0_14px_34px_rgba(var(--ink-rgb),0.07)] max-lg:grid-cols-[minmax(120px,0.32fr)_minmax(0,0.68fr)] max-[820px]:grid-cols-1!" key={item.id}>
                <Link
                  className={cn("relative grid min-h-full content-between p-[22px] max-sm:min-h-[140px] max-sm:p-5 [&>span]:text-[0.88rem] [&>span]:leading-[1.35] [&>span]:font-extrabold [&>span]:text-[rgba(var(--ink-rgb),0.64)] [&>strong]:font-[var(--font-latin-rounded)] [&>strong]:text-[clamp(3rem,6vw,5rem)] [&>strong]:leading-[0.9] [&>strong]:font-black [&>strong]:tracking-[-0.08em] [&>strong]:text-primary max-sm:[&>strong]:text-[3.2rem] [&>svg]:size-7 [&>svg]:text-primary", recapMediaToneClassNames[index % recapMediaToneClassNames.length])}
                ...
                <div className="aspect-[4/3] w-full self-center overflow-hidden bg-[rgba(var(--accent-rgb),0.08)] max-[820px]:aspect-[16/10] [&_img]:block [&_img]:size-full [&_img]:object-cover">
                  {item.imageUrl ? (
                    <RevealImage
                    ...
                    <div className="grid size-full place-items-center bg-[rgba(var(--accent-rgb),0.08)] p-6 font-extrabold text-[rgba(var(--ink-rgb),0.7)]">活动图片待补充</div>
                  )}
                </div>

                <div className="grid min-w-0 content-center gap-3.5 p-7 max-lg:col-span-full max-sm:p-5">
                  <div className="flex flex-wrap gap-2">
                    <span
                      className={cn(
                        "inline-flex min-h-8 items-center rounded-[var(--radius-pill)] border px-[11px] text-[0.86rem] font-[750] shadow-[inset_4px_0_0_rgba(var(--accent-rgb),0.68)]",
                    ...
                    <span className="inline-flex min-h-8 items-center rounded-[var(--radius-pill)] bg-[rgba(var(--accent-rgb),0.08)] px-[11px] text-[0.86rem] font-[750] text-[var(--accent-strong)]">{item.locationLabel}</span>
                    <span className="inline-flex min-h-8 items-center rounded-[var(--radius-pill)] bg-[rgba(var(--accent-rgb),0.08)] px-[11px] text-[0.86rem] font-[750] text-[var(--accent-strong)]">
                      {(item.gallery?.length ?? 0) > 0
                        ? `${item.gallery?.length ?? 0} 张照片`
                        : "已归档"}
                    </span>
                  </div>
                  <h2 className="m-0 text-[clamp(1.6rem,2.8vw,2.35rem)] leading-[1.12] font-[720] tracking-[-0.048em] text-[#111a1d]">{item.title}</h2>
                  <p className="m-0 leading-[1.76] text-[rgba(var(--ink-rgb),0.66)]">{item.summary}</p>
                  <Link
                    href={`/events/${item.slug}`}
                    prefetch={false}
                    className="inline-flex w-fit items-center gap-2 font-black text-primary [&_svg]:size-4"
                  >
                    查看活动详情
                    <ArrowRight aria-hidden="true" strokeWidth={2} />
                  </Link>
