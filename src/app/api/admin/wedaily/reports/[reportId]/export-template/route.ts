...
import {
  fetchAdminWeDailyReportExportTemplate,
  previewAdminWeDailyReportExportTemplate,
...
export async function POST(
  ...
  if (!markdown.trim()) {
    return NextResponse.json({ error: "empty_markdown" }, { status: 400 });
  }

  try {
    const template = await previewAdminWeDailyReportExportTemplate(parsedReportId, markdown);
