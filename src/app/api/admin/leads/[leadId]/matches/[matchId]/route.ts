...
export async function PATCH(
  request: Request,
  context: { params: Promise<{ leadId: string; matchId: string }> },
) {
  ...
  const { leadId, matchId } = await context.params;
  ...
  if (!leadId || !matchId || !memberId || !status) {
...
export async function DELETE(
  ...
  revalidateAdminLeadPaths(leadId);
  return NextResponse.json({ saved: "lead_match_deleted" });
}
