...
export function AdminWeDailyReportsClient({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const template = await fetchCurrentExportTemplate(selectedReport.id, markdown);
        if (!cancelled) {
...
async function fetchCurrentExportTemplate(reportId: number, markdown: string) {
  const response = await fetch(`/api/admin/wedaily/reports/${reportId}/export-template`, {
    body: JSON.stringify({ markdown }),
    cache: "no-store",
    headers: {
