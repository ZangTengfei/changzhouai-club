...
const backgroundPaths = [
  "public/share-cards/wedaily-neural-glass.webp",
  "public/share-cards/wedaily-intelligent-grid.webp",
  "public/share-cards/wedaily-holographic-orbit.webp",
];
const limits = {
  backgroundBytes: 100_000,
  firstLoadBytes: 900_000,
  gzipBytes: 350_000,
  largestChunkBytes: 300_000,
};

for (const filePath of [pagePath, clientPath, statsPath, ...backgroundPaths]) {
...
if (/\/share-cards\/wedaily-[^"']+\.png/.test(clientSource)) {
  throw new Error("share_page_backgrounds_must_use_webp");
}
...
const backgroundBytes = backgroundPaths.reduce(
  (sum, backgroundPath) => sum + statSync(backgroundPath).size,
  0,
);
...
if (backgroundBytes > limits.backgroundBytes) {
  throw new Error(`background_budget_exceeded:${backgroundBytes}`);
}
